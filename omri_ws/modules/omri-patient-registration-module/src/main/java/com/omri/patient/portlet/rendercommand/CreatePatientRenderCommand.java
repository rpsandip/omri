...
@Component(property = { "javax.portlet.name=com_omri_patient_portlet_OmriPatientRegistrationModulemvcportletPortlet",
		"mvc.command.name=/create-patient" }, service = MVCRenderCommand.class)
public class CreatePatientRenderCommand implements MVCRenderCommand {
	...
	public String render(RenderRequest renderRequest, RenderResponse renderResponse) throws PortletException {
		...
		try {
			...
			isClinicAdmin = UserGroupRoleLocalServiceUtil.hasUserGroupRole(themeDisplay.getUserId(), orgGroupId,
					clinicAdminRole.getRoleId());
	...
	private void setPatientBeanForUpdatePatient(RenderRequest renderRequest) {
		...
		try {
			...
			Patient_Clinic patientClinic = Patient_ClinicLocalServiceUtil
					.getPatientClinicByPatientId(patient.getPatientId());
			...
			if (Validator.isNotNull(patientClinic)) {
				List<Patient_Clinic_Resource> patientClinicResourceList = Patient_Clinic_ResourceLocalServiceUtil
						.getPatientClinicByPatiendIdAndClinicId(patientClinic.getPatientId(),
								patientClinic.getClinicId());
				for (Patient_Clinic_Resource patientClinicResource : patientClinicResourceList) {
					...
					Specification specification = SpecificationLocalServiceUtil
							.getSpecification(patientClinicResource.getSpecificationId());
					Clinic clinic = ClinicLocalServiceUtil.getClinic(patientClinic.getClinicId());
					PatientResourceBean patientResourceBean = new PatientResourceBean(patient, clinic, resource,
							specification, patientClinicResource);
	...
	private void setClinicResourcesForEditPatient(Patient_Clinic patientClinic, RenderRequest renderRequest) {
		List<Clinic_Resource> clinicResourcesList = Clinic_ResourceLocalServiceUtil
				.getClinicResources(patientClinic.getClinicId());
		...
		for (Clinic_Resource clinicResource : clinicResourcesList) {
			...
			List<Clinic_Resource> resourceSpecificationList = Clinic_ResourceLocalServiceUtil
					.getClinicResources(patientClinic.getClinicId(), clinicResource.getResourceId());
			...
			for (Clinic_Resource clinicResourceSpecification : resourceSpecificationList) {
				try {
					Resource_Specification resourceSpecification = Resource_SpecificationLocalServiceUtil
							.getResourceSpecification(clinicResourceSpecification.getResourceId(),
									clinicResourceSpecification.getSpecificationId());
					...
					specJsonObj.put("specificatonName", clinicResourceSpecification.getSpecificationName() + "("
							+ resourceSpecification.getCptCode() + ")");
